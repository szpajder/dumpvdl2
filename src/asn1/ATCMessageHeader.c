/*
 * Generated by asn1c-0.9.28 (http://lionet.info/asn1c)
 * From ASN.1 module "CPDLCMessageSetVersion2BCVersion1"
 * 	found in "../../../dumpvdl2.asn1/atn-b2_cpdlc-v1.asn1"
 * 	`asn1c -fcompound-names -fincludes-quoted -gen-PER`
 */

#include "ATCMessageHeader.h"

static int asn_DFL_5_set_1(int set_value, void **sptr) {
	LogicalAck_t *st = *sptr;
	
	if(!st) {
		if(!set_value) return -1;	/* Not a default value */
		st = (*sptr = CALLOC(1, sizeof(*st)));
		if(!st) return -1;
	}
	
	if(set_value) {
		/* Install default value 1 */
		*st = 1;
		return 0;
	} else {
		/* Test default value 1 */
		return (*st == 1);
	}
}
static asn_TYPE_member_t asn_MBR_ATCMessageHeader_1[] = {
	{ ATF_NOFLAGS, 0, offsetof(struct ATCMessageHeader, messageIdNumber),
		(ASN_TAG_CLASS_CONTEXT | (0 << 2)),
		+1,	/* EXPLICIT tag at current level */
		&asn_DEF_MsgIdentificationNumber,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"messageIdNumber"
		},
	{ ATF_POINTER, 1, offsetof(struct ATCMessageHeader, messageRefNumber),
		(ASN_TAG_CLASS_CONTEXT | (1 << 2)),
		+1,	/* EXPLICIT tag at current level */
		&asn_DEF_MsgReferenceNumber,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"messageRefNumber"
		},
	{ ATF_NOFLAGS, 0, offsetof(struct ATCMessageHeader, dateTime),
		(ASN_TAG_CLASS_CONTEXT | (2 << 2)),
		+1,	/* EXPLICIT tag at current level */
		&asn_DEF_DateTimeGroup,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"dateTime"
		},
	{ ATF_POINTER, 1, offsetof(struct ATCMessageHeader, logicalAck),
		(ASN_TAG_CLASS_CONTEXT | (3 << 2)),
		+1,	/* EXPLICIT tag at current level */
		&asn_DEF_LogicalAck,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		asn_DFL_5_set_1,	/* DEFAULT 1 */
		"logicalAck"
		},
};
static const int asn_MAP_ATCMessageHeader_oms_1[] = { 1, 3 };
static const ber_tlv_tag_t asn_DEF_ATCMessageHeader_tags_1[] = {
	(ASN_TAG_CLASS_UNIVERSAL | (16 << 2))
};
static const asn_TYPE_tag2member_t asn_MAP_ATCMessageHeader_tag2el_1[] = {
    { (ASN_TAG_CLASS_CONTEXT | (0 << 2)), 0, 0, 0 }, /* messageIdNumber */
    { (ASN_TAG_CLASS_CONTEXT | (1 << 2)), 1, 0, 0 }, /* messageRefNumber */
    { (ASN_TAG_CLASS_CONTEXT | (2 << 2)), 2, 0, 0 }, /* dateTime */
    { (ASN_TAG_CLASS_CONTEXT | (3 << 2)), 3, 0, 0 } /* logicalAck */
};
static asn_SEQUENCE_specifics_t asn_SPC_ATCMessageHeader_specs_1 = {
	sizeof(struct ATCMessageHeader),
	offsetof(struct ATCMessageHeader, _asn_ctx),
	asn_MAP_ATCMessageHeader_tag2el_1,
	4,	/* Count of tags in the map */
	asn_MAP_ATCMessageHeader_oms_1,	/* Optional members */
	2, 0,	/* Root/Additions */
	-1,	/* Start extensions */
	-1	/* Stop extensions */
};
asn_TYPE_descriptor_t asn_DEF_ATCMessageHeader = {
	"ATCMessageHeader",
	"ATCMessageHeader",
	SEQUENCE_free,
	SEQUENCE_print,
	SEQUENCE_constraint,
	SEQUENCE_decode_ber,
	SEQUENCE_encode_der,
	SEQUENCE_decode_xer,
	SEQUENCE_encode_xer,
	SEQUENCE_decode_uper,
	SEQUENCE_encode_uper,
	0,	/* Use generic outmost tag fetcher */
	asn_DEF_ATCMessageHeader_tags_1,
	sizeof(asn_DEF_ATCMessageHeader_tags_1)
		/sizeof(asn_DEF_ATCMessageHeader_tags_1[0]), /* 1 */
	asn_DEF_ATCMessageHeader_tags_1,	/* Same as above */
	sizeof(asn_DEF_ATCMessageHeader_tags_1)
		/sizeof(asn_DEF_ATCMessageHeader_tags_1[0]), /* 1 */
	0,	/* No PER visible constraints */
	asn_MBR_ATCMessageHeader_1,
	4,	/* Elements count */
	&asn_SPC_ATCMessageHeader_specs_1	/* Additional specs */
};

