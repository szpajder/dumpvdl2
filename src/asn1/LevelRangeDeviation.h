/*
 * Generated by asn1c-0.9.28 (http://lionet.info/asn1c)
 * From ASN.1 module "ADSMessageSetVersion2"
 * 	found in "../../../dumpvdl2.asn1/atn-b2_adsc_v2.asn1"
 * 	`asn1c -fcompound-names -fincludes-quoted -gen-PER`
 */

#ifndef	_LevelRangeDeviation_H_
#define	_LevelRangeDeviation_H_


#include "asn_application.h"

/* Including external dependencies */
#include "LevelRangeDeviationCeiling.h"
#include "LevelRangeDeviationFloor.h"
#include "LevelRangeDeviationBoth.h"
#include "constr_CHOICE.h"

#ifdef __cplusplus
extern "C" {
#endif

/* Dependencies */
typedef enum LevelRangeDeviation_PR {
	LevelRangeDeviation_PR_NOTHING,	/* No components present */
	LevelRangeDeviation_PR_ceiling,
	LevelRangeDeviation_PR_floor,
	LevelRangeDeviation_PR_both
} LevelRangeDeviation_PR;

/* LevelRangeDeviation */
typedef struct LevelRangeDeviation {
	LevelRangeDeviation_PR present;
	union LevelRangeDeviation_u {
		LevelRangeDeviationCeiling_t	 ceiling;
		LevelRangeDeviationFloor_t	 floor;
		LevelRangeDeviationBoth_t	 both;
	} choice;
	
	/* Context for parsing across buffer boundaries */
	asn_struct_ctx_t _asn_ctx;
} LevelRangeDeviation_t;

/* Implementation */
extern asn_TYPE_descriptor_t asn_DEF_LevelRangeDeviation;

#ifdef __cplusplus
}
#endif

#endif	/* _LevelRangeDeviation_H_ */
#include "asn_internal.h"
