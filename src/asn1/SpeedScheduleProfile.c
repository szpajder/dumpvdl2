/*
 * Generated by asn1c-0.9.28 (http://lionet.info/asn1c)
 * From ASN.1 module "ADSMessageSetVersion2"
 * 	found in "../../../dumpvdl2.asn1/atn-b2_adsc_v2.asn1"
 * 	`asn1c -fcompound-names -fincludes-quoted -gen-PER`
 */

#include "SpeedScheduleProfile.h"

static asn_TYPE_member_t asn_MBR_SpeedScheduleProfile_1[] = {
	{ ATF_POINTER, 4, offsetof(struct SpeedScheduleProfile, predicted_gross_mass_at_tod),
		(ASN_TAG_CLASS_CONTEXT | (0 << 2)),
		+1,	/* EXPLICIT tag at current level */
		&asn_DEF_PredictedGrossMassAtToD,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"predicted-gross-mass-at-tod"
		},
	{ ATF_POINTER, 3, offsetof(struct SpeedScheduleProfile, climbSpeed),
		(ASN_TAG_CLASS_CONTEXT | (1 << 2)),
		+1,	/* EXPLICIT tag at current level */
		&asn_DEF_ClimbSpeed,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"climbSpeed"
		},
	{ ATF_POINTER, 2, offsetof(struct SpeedScheduleProfile, initialCruiseSpeedAtTOC),
		(ASN_TAG_CLASS_CONTEXT | (2 << 2)),
		+1,	/* EXPLICIT tag at current level */
		&asn_DEF_InitialCruiseSpeedAtToC,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"initialCruiseSpeedAtTOC"
		},
	{ ATF_POINTER, 1, offsetof(struct SpeedScheduleProfile, finalCruiseSpeedAtToD),
		(ASN_TAG_CLASS_CONTEXT | (3 << 2)),
		+1,	/* EXPLICIT tag at current level */
		&asn_DEF_FinalCruiseSpeedAtToD,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"finalCruiseSpeedAtToD"
		},
	{ ATF_NOFLAGS, 0, offsetof(struct SpeedScheduleProfile, descentSpeed),
		(ASN_TAG_CLASS_CONTEXT | (4 << 2)),
		+1,	/* EXPLICIT tag at current level */
		&asn_DEF_DescentSpeed,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"descentSpeed"
		},
};
static const int asn_MAP_SpeedScheduleProfile_oms_1[] = { 0, 1, 2, 3 };
static const ber_tlv_tag_t asn_DEF_SpeedScheduleProfile_tags_1[] = {
	(ASN_TAG_CLASS_UNIVERSAL | (16 << 2))
};
static const asn_TYPE_tag2member_t asn_MAP_SpeedScheduleProfile_tag2el_1[] = {
    { (ASN_TAG_CLASS_CONTEXT | (0 << 2)), 0, 0, 0 }, /* predicted-gross-mass-at-tod */
    { (ASN_TAG_CLASS_CONTEXT | (1 << 2)), 1, 0, 0 }, /* climbSpeed */
    { (ASN_TAG_CLASS_CONTEXT | (2 << 2)), 2, 0, 0 }, /* initialCruiseSpeedAtTOC */
    { (ASN_TAG_CLASS_CONTEXT | (3 << 2)), 3, 0, 0 }, /* finalCruiseSpeedAtToD */
    { (ASN_TAG_CLASS_CONTEXT | (4 << 2)), 4, 0, 0 } /* descentSpeed */
};
static asn_SEQUENCE_specifics_t asn_SPC_SpeedScheduleProfile_specs_1 = {
	sizeof(struct SpeedScheduleProfile),
	offsetof(struct SpeedScheduleProfile, _asn_ctx),
	asn_MAP_SpeedScheduleProfile_tag2el_1,
	5,	/* Count of tags in the map */
	asn_MAP_SpeedScheduleProfile_oms_1,	/* Optional members */
	4, 0,	/* Root/Additions */
	-1,	/* Start extensions */
	-1	/* Stop extensions */
};
asn_TYPE_descriptor_t asn_DEF_SpeedScheduleProfile = {
	"SpeedScheduleProfile",
	"SpeedScheduleProfile",
	SEQUENCE_free,
	SEQUENCE_print,
	SEQUENCE_constraint,
	SEQUENCE_decode_ber,
	SEQUENCE_encode_der,
	SEQUENCE_decode_xer,
	SEQUENCE_encode_xer,
	SEQUENCE_decode_uper,
	SEQUENCE_encode_uper,
	0,	/* Use generic outmost tag fetcher */
	asn_DEF_SpeedScheduleProfile_tags_1,
	sizeof(asn_DEF_SpeedScheduleProfile_tags_1)
		/sizeof(asn_DEF_SpeedScheduleProfile_tags_1[0]), /* 1 */
	asn_DEF_SpeedScheduleProfile_tags_1,	/* Same as above */
	sizeof(asn_DEF_SpeedScheduleProfile_tags_1)
		/sizeof(asn_DEF_SpeedScheduleProfile_tags_1[0]), /* 1 */
	0,	/* No PER visible constraints */
	asn_MBR_SpeedScheduleProfile_1,
	5,	/* Elements count */
	&asn_SPC_SpeedScheduleProfile_specs_1	/* Additional specs */
};

